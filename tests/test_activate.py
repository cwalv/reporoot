"""Tests for reporoot activate/reset commands and integration registry dispatch."""
...
from reporoot.activate import reset, run
...
class TestActivate:
    def test_sets_rr_active(self, workspace: Path):
        ...
        active = (workspace / ".rr-active").read_text().strip()
        assert active == "myproject"

    def test_deactivate(self, workspace: Path):
        os.chdir(workspace)
        (workspace / ".rr-active").write_text("something\n")
        (workspace / "package.json").write_text("{}")

        run(project="none")

        assert not (workspace / ".rr-active").exists()
        assert not (workspace / "package.json").exists()

    def test_reset(self, workspace: Path):
        os.chdir(workspace)
        (workspace / ".rr-active").write_text("something\n")
        (workspace / "package.json").write_text("{}")
